...
```{r}
plot(pdp, geom = "variable", variables = "karno", times = 80)
```
...
```{r}
plot(pdp, geom = "variable", variables = "celltype", times = 80)
```

Of course, the plots can be prepared for multiple timepoints, at the same time and presented on one plot.

```{r}
plot(pdp, geom = "variable", variables = "karno", times = c(1, 80, 151.72))
```

```{r}
plot(pdp, geom = "variable", variables = "celltype", times = c(1, 80, 151.72))
```
