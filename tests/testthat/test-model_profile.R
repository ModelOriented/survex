...
test_that("model_profile with type = 'partial' and output_type = 'chf' works", {
    veteran <- survival::veteran[c(1:3, 16:18, 46:48, 56:58, 71:73, 91:93, 111:113, 126:128), ]
    ...
    mp_cph_cat <- model_profile(cph_exp, output_type = 'chf', variable_splits_type = "quantiles", grid_points = 6, N = 4)
    plot(mp_cph_cat, variables = "celltype", variable_type = "categorical")
    ...
    mp_chosen_var <- model_profile(cph_exp, output_type = 'chf', variable_splits_type = "quantiles", grid_points = 6, variables = "karno")
    expect_s3_class(mp_chosen_var, "model_profile_survival")
    expect_true(all(mp_chosen_var$eval_times == cph_exp$times))
    expect_equal(ncol(mp_chosen_var$result), 7)

    mp_cph_num <- model_profile(cph_exp, output_type = 'chf', variable_splits_type = "quantiles", grid_points = 6)
    plot(mp_cph_num, variable_type = "numerical")
    plot(mp_cph_num, numerical_plot_type = "contours")
    ...
    mp_rsf_cat <- model_profile(rsf_ranger_exp, output_type = 'chf', variable_splits_type = "uniform", grid_points = 6)
    plot(mp_rsf_cat, variable_type = "categorical")
    ...
    mp_rsf_num <- model_profile(rsf_ranger_exp, output_type = 'chf', variable_splits_type = "uniform", grid_points = 6)
    plot(mp_rsf_num, variable_type = "numerical")
    plot(mp_rsf_num, variable_type = "numerical", numerical_plot_type = "contours")
    ...
    centered_mp <- model_profile(rsf_src_exp, output_type = 'chf', variables = "karno", center = TRUE)
    plot(centered_mp, geom = "variable", variables = "karno", times = rsf_src_exp$times[1])

})

test_that("model_profile with type = 'accumulated' and output_type = 'chf' works", {
    veteran <- survival::veteran[c(1:3, 16:18, 46:48, 56:58, 71:73, 91:93, 111:113, 126:128), ]
    ...
    expect_output(print(mp_cph_num))
    expect_error(plot(mp_rsf_num, output_type = 'chf', variables = "nonexistent", grid_points = 6))
})
