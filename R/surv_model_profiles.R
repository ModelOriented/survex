...
surv_ale <- function(x, ...,
    ...
    test_explainer(x, has_data = TRUE, has_survival = TRUE, function_name = "surv_ale")
    ...
    profiles <- lapply(variables, function(variable) {
        ...
        quantile_vals <- as.numeric(quantile(data[,variable],
                                             seq(0.01, 1, length.out = grid_points),
                                             type = 1))

        # Quantile points vector
        quantile_vec <- c(min(data[,variable]), quantile_vals)
        ...
        interval_index <- findInterval(data[,variable], quantile_vec, left.open = TRUE)
        ...
        predictions_lower = predict_survival_function(model = model,
                                             newdata = X_lower,
                                             times = times)
        predictions_upper = predict_survival_function(model = model,
                                             newdata = X_upper,
                                             times = times)
        ...
        ale_means <- aggregate(yhat_cumsum ~ time, data = deltas, FUN = function(x) {
    ...
    class(profiles) <- c("aggregated_survival_profiles_explainer",
